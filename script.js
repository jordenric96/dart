...
// De 7 actieve spelers (Iedereen tegen iedereen = 21 matchen)
...
navButtons.forEach(btn => {
    btn.addEventListener('click', (e) => {
        ...
        // Verberg de navigatiebalk op de tablets om fullscreen ruimte te winnen
...
document.getElementById('reset-btn').addEventListener('click', async () => {
    let code = prompt("Voer de mastercode in om het toernooi volledig te resetten (1403):");
    if(code === "1403") {
        genereerSpeelschema();
        initStats();
        await saveState(true);
        alert("Database succesvol leeggemaakt!");
    }
});

// --- TOERNOOI SCHEMATISERING ---
function genereerSpeelschema() {
    state.matches = [];
    let matchId = 1;
    // Round Robin generator: iedereen speelt exact 1x tegen elkaar
...
// --- 1. TV DASHBOARD SCHERM (LAPTOP WEERGAVE) ---
function renderDashboard() {
    ...
    // Bereken alle live statistieken
                ...
                <div class="card" style="flex: 1;">
                    ...
                    <div class="match-list-container">
                        ${state.matches.map(m => {
                            ...
                            return `<div class="match-item ${cls}"><span><strong>${m.p1}</strong> vs <strong>${m.p2}</strong></span><span>${label}</span></div>`;
                        }).join('')}
                    </div>
                </div>
            </div>
            ...
            <div class="grid-col">
                <div class="card" style="height: 100%; justify-content: space-around;">
                    ...
                    <div>
                        ...
                        <table class="retro-table">
                            <tr><th>Speler</th><th>Checkout</th></tr>
                            ${getTopCheckouts().length === 0 ? `<tr><td colspan="2">-</td></tr>` : getTopCheckouts().slice(0,3).map(co => `<tr><td>${co.naam}</td><td><strong>${co.score}</strong></td></tr>`).join('')}
                        </table>
                    </div>

                    <div>
                        <h3>⏱️ Gem. Pijlen Per Leg (Totaal Geworpen)</h3>
                        <table class="retro-table">
                            <tr><th>Speler</th><th>Pijlen/Leg</th><th>Totaal</th></tr>
                            ${dartsPerLeg.slice(0,3).map(d => `<tr><td>${d.naam}</td><td><strong>${d.dpl}</strong></td><td>${d.total}</td></tr>`).join('')}
                        </table>
                    </div>

                    <div>
                        <h3>🐂 Meeste Bulls Gewonnen (Startbeurt)</h3>
                        <table class="retro-table">
...
function generateLiveBoardHTML(title, match) {
    if (!match) return `<div class="live-board"><h3>${title}</h3><p style="font-size: 2em; color:#ccc; margin:0;">Bord Vrij</p></div>`;
    if (match.status === 'bullen') return `<div class="live-board"><h3>${title}</h3><div class="live-score" style="font-size: 2.5em;">🐂 BULLEN VOOR START 🐂</div><p style="margin:0;">${match.p1} vs ${match.p2}</p></div>`;
    return `
        <div class="live-board">
            <h3>${title} <span class="live-legs">First to 3</span></h3>
...
// --- 2. TABLET 501 MATCH ENGINE (REKENMACHINE) ---
function renderTablet(boardId) {
    const actieveMatch = state.matches.find(m => m.board === boardId && m.status !== 'finished');

    if (!actieveMatch) {
        let html = `<div class="tablet-view"><h2 class="board-header">Selecteer wedstrijd op ${boardId === 'board1' ? 'BORD 1' : 'BORD 2'}</h2><div class="match-selector">`;
        const waitingMatches = state.matches.filter(m => m.status === 'waiting');
        if(waitingMatches.length === 0) { html += `<h2>Alle wedstrijden gespeeld!</h2></div></div>`; appContainer.innerHTML = html; return; }
    ...
    if (actieveMatch.status === 'bullen') {
        showModal(`
            <h2 style="font-size:2.5em; margin:0;">🐂 BULL EYE TREKKING 🐂</h2>
            <p style="font-size: 1.5em;">Wie gooide het dichtst bij de bullseye en mag beginnen?</p>
        ...
        let html = `
            <div class="tablet-view">
                <div style="display:flex; justify-content: space-between; align-items:center;">
                    <h2 class="board-header">BORD ${boardId === 'board1' ? '1' : '2'} (501 - FIRST TO 3)</h2>
                    <button class="retro-button danger" style="padding: 5px 15px; font-size:1.2rem;" onclick="abortMatch('${actieveMatch.id}')">Afbreken</button>
                </div>
                ...
                <div class="scoring-area">
                    ...
                    <div class="input-section">
                        <label style="font-size: 2.5em; font-family:'Alfa Slab One', serif; color:white; margin-right:10px;">SCORE:</label>
                        <input type="number" id="score-input" class="score-input-field" placeholder="0" min="0" max="180">
                        <button class="retro-button success" style="font-size: 2.5em; padding: 5px 35px;" onclick="submitScore('${actieveMatch.id}')">OK</button>
                    </div>
                </div>
            </div>
        `;
...
window.abortMatch = async function(matchId) {
    if(confirm("Weet je zeker dat je deze wedstrijd wilt annuleren? Alle scores worden gereset.")) {
        const m = state.matches.find(x => x.id === matchId);
...
window.setStarter = async function(matchId, playerNum) {
    ...
    // Voeg bull-statistiek toe (alleen bij de allereerste startbepaling van de match)
...
window.submitScore = async function(matchId) {
    ...
    if (isNaN(throwScore) || throwScore < 0 || throwScore > 180) {
        alert("Voer een geldige score in tussen 0 en 180!"); input.value = ''; return;
    }
    ...
    if (newScore < 0 || newScore === 1) {
        ...
        setTimeout(() => alert("💥 BUST! Score telt niet."), 100);
        return;
    }
    ...
    if (newScore === 0) {
        ...
        showModal(`
            <h2 style="color:var(--green-ok); font-size:3em; margin:0;">🎯 FINISH!</h2>
            <p style="font-size: 1.5em;">Met hoeveel pijlen smeet je de leg uit in deze <b>LAATSTE BEURT</b>?</p>
            <div style="display:flex; justify-content:center; gap:20px; margin-top:20px;">
